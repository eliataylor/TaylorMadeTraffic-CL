...
class UpworkJobModel extends CI_Model
{
    ...
    {
        $lastFetchedJobTitles = $this->getLastFetchedJobTitles();
    ...
    private function getLastFetchedJobTitles()
    {
        if (file_exists($this->rssFilePath)) {
            $rssContent = file_get_contents($this->rssFilePath);
            preg_match_all('/<title>(.*?)<\/title>/', $rssContent, $matches);
            return $matches[1];
        }
